...
#let icon(
  ...
  let cfg = get-config()
  // 联系方式统一使用中性深灰，避免随模板主题显示为抢眼的彩色图标。
  let actual-color = if name in ("phone", "email", "location") { rgb("#4b5563") } else if color == auto { cfg.colors.primary } else { color }
  ...
  box(
    ...
    align(
      center + horizon,
      text(
        // 位置符号使用系统符号字体；电话和邮箱使用 Wingdings 的单色字形。
        font: if name == "location" { "Apple Symbols" } else { nerd-font },
        fill: actual-color,
        size: size,
        char,
      ),
