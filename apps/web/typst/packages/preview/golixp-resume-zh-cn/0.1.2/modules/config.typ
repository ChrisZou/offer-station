...
#let _default-config = (
  ...
  fonts: (
    ...
    // Wingdings 随 macOS 编译环境提供，可稳定显示简历常用的单色联系方式符号。
    nerd: "Wingdings",
  ...
  icon-chars : (
    // 联系方式类图标
    // 简历常用的单色联系方式符号；不依赖彩色 Emoji 或图标字体。
    phone: "(",
    email: "*",
    location: "⌖",
    link: "\u{f0c1}",            // 
...
#let _finalize-config(base, overrides: (:)) = {
  ...
  let icon-chars = _merge-dict(
    base.at("icon-chars"),
    if "icon-chars" in overrides { overrides.at("icon-chars") } else { (:) },
  )
