...
#let setup-styles(
  ...
  let resume-header(
    ...
    let contacts = ()
    if telephone != "" {
      // The template's Nerd Font dependency is not bundled in the desktop
      // compiler. Use stable Unicode symbols so contact icons never degrade
      // to missing-glyph squares in exported Chinese PDFs.
      contacts.push([☎ #h(0.2em) #telephone])
    }
    if wechat-id != "" {
