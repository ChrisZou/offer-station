<template>
  <div class="design-box" :class="{ 'workbench-design': workbenchMode }">
    <!-- 导航栏 -->
    <design-nav
      ref="navRef"
    ...
    <div class="bottom">
      <!-- 左侧添加模块区域 -->
      <div v-if="!workbenchMode" ref="leftRef" class="left">
      ...
      <!-- 预览区域 -->
      <div id="print" ref="centerRef" class="center">
        <!-- 放大缩小 -->
...
<script setup lang="ts">
  ...
  import { createWorkbenchResume, loadWorkbenchResume, saveWorkbenchResume } from '@/workbench/resumeBridge';
  ...
  // 静态内置入口使用 hash 路由。直接读取 hash 作为兜底，避免设计器在
  // Vue Router 首次同步前被误判为猫步官网模式并调用原站 API。
  const hashQuery = typeof window === 'undefined'
    ? new URLSearchParams()
    : new URLSearchParams(window.location.hash.split('?')[1] || '');
  const workbenchMode = route.query.workbench === '1' || hashQuery.get('workbench') === '1';
  const centerRef = ref<HTMLElement | null>(null);
  let layoutObserver: ResizeObserver | null = null;
  let sourceCreatedAt = '';
  let sourceResume: any;

  const receiveWorkbenchMessage = (event: MessageEvent) => {
    if (event.data?.type !== 'JOB_WORKBENCH_SET_RESUME') return;
    sourceResume = event.data.resume;
    sourceCreatedAt = sourceResume?.createdAt || '';
    const nativeResume = event.data.nativeResume || loadWorkbenchResume(sourceResume);
    changeResumeJsonData(nativeResume);
    saveWorkbenchResume(nativeResume, sourceCreatedAt);
    setUuid();
  };
  ...
  const resetStoreAndLocal = async (isReset = false, ID = id) => {
    if (workbenchMode) {
      const next = isReset ? createWorkbenchResume(sourceResume) : loadWorkbenchResume(sourceResume);
      changeResumeJsonData(next);
      saveWorkbenchResume(next, sourceCreatedAt);
      setUuid();
      return;
    }
  ...
  if (!workbenchMode) resetStoreAndLocal();
  ...
  onMounted(async () => {
    if (workbenchMode) {
      ...
      window.parent.postMessage({ type: 'JOB_WORKBENCH_STUDIO_READY' }, '*');
      window.setTimeout(() => {
        if (!resumeJsonNewStore.value.COMPONENTS.length) changeResumeJsonData(loadWorkbenchResume());
      }, 500);
    }
    resizeDOM();
    initClickListen();
    ...
    await nextTick();
    updateAutoFit();
    if (workbenchMode && centerRef.value) {
      layoutObserver = new ResizeObserver(updateAutoFit);
      layoutObserver.observe(centerRef.value);
    }
  ...
  onBeforeUnmount(() => {
    ...
    if (workbenchMode) window.removeEventListener('message', receiveWorkbenchMessage);
  });
  ...
  const generateReportNew = async () => {
    if (!workbenchMode) addMakeResumeCountAsync(); // 工作台模式不依赖猫步后端
  ...
  const autoFitScale = ref<number>(1);
  const effectiveScale = computed(() => workbenchMode
    ? Math.min(sizeCenter.value, autoFitScale.value)
    : sizeCenter.value);
  const updateAutoFit = () => {
    if (!workbenchMode || !centerRef.value) {
    ...
    const availableWidth = Math.max(0, centerRef.value.clientWidth - 32);
    autoFitScale.value = Math.max(0.5, Math.min(1, availableWidth / 820));
  };
...
<style lang="scss" scoped>
  ...
  .design-box.workbench-design {
    ...
    .bottom > .left {
      ...
      flex: 0 0 clamp(218px, 17vw, 252px);
      height: calc(100vh - 64px);
      overflow-x: hidden;
    ...
    .bottom > .config {
      width: clamp(360px, 27vw, 420px);
      flex: 0 0 clamp(360px, 27vw, 420px);
      height: calc(100vh - 64px);
      min-width: 0;
      overflow-x: hidden;
    ...
    :deep(.el-backtop) { right: 24px !important; }
  }
