...
export default defineConfig(async ({ command, mode }: ConfigEnv): Promise<UserConfig> => {
  ...
  const isWorkbenchBuild = mode === 'workbench';
  const env = loadEnv(mode, root);
  const viteEnv = wrapperEnv(env);
  const { VITE_OUTPUT_DIR } = viteEnv;
  const isProduction = viteEnv.VITE_ENV === 'production';
  const isDev = !isProduction;
  const outputDir = isWorkbenchBuild
    ? path.resolve(__dirname, '../web/public/resume-studio')
    : VITE_OUTPUT_DIR;
  ...
  return {
    base: isWorkbenchBuild ? '/resume-studio/' : '/',
    resolve: {
      alias: {
    ...
    plugins: [
      ...
      // 工作台内置构建只有一个静态入口，不需要启动 Puppeteer 预渲染官网首页。
      ...(isWorkbenchBuild ? [] : [prerender({
